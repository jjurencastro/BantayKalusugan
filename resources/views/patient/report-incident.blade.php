    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg border border-red-100 dark:border-slate-700">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ __('Incident Reporting Form') }}</h3>
                    <p class="text-gray-600 dark:text-slate-400 mb-6">{{ __('Use this form to report a health incident or urgent medical condition that should be reviewed by a doctor.') }}</p>
                    ...
                    <form action="{{ route('patient.store-incident') }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <select id="incident_type" name="incident_type" required class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500">
                                ...
                                <option value="injury" @selected(old('incident_type') === 'injury')>{{ __('Injury') }}</option>
                                <option value="illness" @selected(old('incident_type') === 'illness')>{{ __('Illness') }}</option>
                                <option value="emergency" @selected(old('incident_type') === 'emergency')>{{ __('Emergency') }}</option>
                                <option value="follow_up" @selected(old('incident_type') === 'follow_up')>{{ __('Follow-up Consultation') }}</option>
                                <option value="medication_issue" @selected(old('incident_type') === 'medication_issue')>{{ __('Medication Issue') }}</option>
                                <option value="other" @selected(old('incident_type') === 'other')>{{ __('Other') }}</option>
                            </select>
                            @error('incident_type')
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">{{ __('E.g., fever, headache, chest pain, shortness of breath, etc.') }}</p>
                            @error('symptoms')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="5" required placeholder="{{ __('Provide details about what happened, when it started, and what support you need...') }}"
                    ...
                    <div class="mt-8 pt-8 border-t border-gray-200 dark:border-slate-700">
                        ...
                        <p class="text-gray-600 dark:text-slate-400">
                            {{ __('For life-threatening emergencies, call 911 immediately.') }}
                        </p>
                    </div>
                </div>
            </div>
